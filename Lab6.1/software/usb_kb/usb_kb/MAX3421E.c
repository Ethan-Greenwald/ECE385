...
volatile alt_u32 *SPI = (alt_u32*)0x00;
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	alt_u8 array[2] = {reg + 2, val};
	int return_code = alt_avalon_spi_command(SPI, 0, 2, array, 0, NULL, 0);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
		alt_u8 array[1+nbytes];
		array[0] = reg + 2;
		for(int i = 0; i < nbytes; i++)
			array[1+i] = data[i];

		int return_code = alt_avalon_spi_command(SPI, 0, 1+nbytes, array, 0, NULL, 0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//return val
	alt_u8 read_data;
	int return_code = alt_avalon_spi_command(SPI, 0, 1, &reg, 1, &read_data, 0);
	if(return_code < 0)
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	int return_code = alt_avalon_spi_command(SPI, 0, 1, &reg, nbytes, data, 0);
	if(return_code < 0)
